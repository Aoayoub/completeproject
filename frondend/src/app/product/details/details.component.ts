...
export class DetailsComponent implements OnInit {
  ...
  // Add a property to store the uploaded image file
  ...
  ngOnInit(): void {
    this.route.params
      .pipe(
        switchMap(({ productId: id }) => this.productService.getById(id))
      )
      .subscribe(
        (product) => {
  ...
  deleteHandler(product: Product) {
    this.productService.delete(product._id).subscribe((product) => {
      this.router.navigate(['/'], { queryParams: { notification: 'deleted' } });
    });
  }

  // Add an event handler for the image upload
  onImageUpload(event: any) {
    const file: File = event.target.files[0];
    // Store the uploaded image file
    this.uploadedImage = `${environment.imagePath}/${file.name}`;
  }
  ...
  bidHandler(form) {
    ...
    this.productService
      .addBid(this.product._id, Number(bid))
      .pipe(
        switchMap(() => {
  ...
  commentHandler(form) {
    ...
    this.productService.comment(this.product._id, comment).subscribe((comment) => {
